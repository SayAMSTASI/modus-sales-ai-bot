...
- постоянное Telegram-меню и административные экраны с inline-кнопками;
- безопасное преобразование Markdown в Telegram HTML: заголовки, акценты, код,
  цитаты и списки без отображения служебных `##`;
- стандартное меню `/` с описаниями основных команд;
- несколько базовых администраторов из `ADMIN_TELEGRAM_IDS`;
- назначение администратора выбором активного пользователя из общего списка,
  подтверждение и снятие прав без перезапуска;
...
Обычный пользовательский и административный сценарий работает кнопками. Slash-команды
сохранены как резервный интерфейс для сопровождения и автоматических тестов:
