...
Пароль архива передаётся отдельно через утверждённый корпоративный канал для
секретов. Не размещать пароль в Jira, Git, почте или открытых чатах.
...
До запуска DevOps должен проверить исходящий доступ к Telegram API с выбранного
production-host:
...
9. Пользователь может отправить PDF, DOCX, XLSX, PPTX, CSV, TXT, JPEG, PNG или
   WebP и получить результат анализа.
10. Бот создаёт по запросу PDF, DOCX, XLSX, PPTX, CSV или TXT через OpenAI Code
    Interpreter и отправляет созданный файл в Telegram.
11. Файл или изображение, возвращённые разрешённым MCP-инструментом, передаются
    пользователю с проверкой типа, размера и источника.
