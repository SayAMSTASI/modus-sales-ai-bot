...
- `/new` создаёт запрос обязательной оценки 1–5; после оценки контекст текущего
  диалога удаляется. Без ответа агента контекст очищается сразу.
- Основные пользовательские и административные сценарии доступны через постоянную
  клавиатуру и inline-кнопки. Slash-команды остаются резервным интерфейсом.
- Ответы преобразуются из безопасного подмножества Markdown в Telegram HTML;
  служебные маркеры заголовков пользователю не показываются.
- Дополнительный администратор выбирается только среди зарегистрированных активных
  пользователей и назначается после отдельного подтверждения.
...
- Usage metrics содержат обезличенный HMAC identifier, модель, tokens, duration,
  result и расчётную стоимость без полного текста диалога.
- Отдельный административный журнал хранит Telegram ID, username через `user_access`,
  время, текст вопроса и результат обработки. Ответ агента и OAuth secrets в него не
  записываются; старые записи удаляются worker по retention policy.
- Оценки завершённых диалогов хранятся отдельно и агрегируются как средняя оценка,
  CSAT (доля оценок 4–5) и распределение 1–5.
